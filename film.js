...
const movieSelectedId = location.hash.slice(1);
console.log(movieSelectedId);
const date = new Date();

filmy.forEach((film) => {
  const premierDate = dayjs(film.premiera);
  const diffPremierDays = dayjs(film.premiera).diff(date, "days");
  let premierDayText;

  if (diffPremierDays > 0) {
    premierDayText = `což je za ${diffPremierDays} dnu`;
  } else if (diffPremierDays === 0) {
    premierDayText = `což je dnes`;
  } else if (diffPremierDays < 0) {
  ...
  if (film.id === movieSelectedId) {
						...
						<div class="card-body">
							...
							<p class="card-text">
								<small class="text-muted" id="premiera"
									>Premiéra <strong>${premierDate.format(
                    `D. M. YYYY`
                  )}</strong>, ${premierDayText} 
				  </small>
							</p>
							<h6>Hodnocení</h6>
							<div class="stars">
    ...
    document.querySelector("#prehravac").innerHTML = `
				<video
					controls
					loop
					preload="auto"
